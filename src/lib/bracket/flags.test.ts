...
describe('flag assets', () => {
  ...
  // The `import.meta.glob` in +page.svelte must enumerate flag codes as a
  // literal string, so it can drift from teams.ts. This guards that the brace
  // list is exactly the set of TeamIds — no missing flags, no dead entries.
  it('the +page.svelte flag glob lists exactly the current teams', () => {
    const match = pageSource.match(/flags\/1x1\/\{([^}]+)\}\.svg/);
    expect(match, 'could not find the flag glob pattern in +page.svelte').not.toBeNull();
